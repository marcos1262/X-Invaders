...
class Laser:
    ...
    def draw(self):
        ...
        glRectf(self.position.x - self.width, self.position.y - self.height, self.position.x + self.width, self.position.y + self.height)
...
class Shooting:
    ...
    def __init__(self):
        ...
        self.fundo = gtk.gdk.pixbuf_new_from_file(os.path.join( mydir, "fundo.png"))
        self.logo = gtk.gdk.pixbuf_new_from_file(os.path.join( mydir, "logo.png"))
            ...
            if  not asteroid.is_dead:
                if randint(0,40/self.level) == 0 and not isinstance(asteroid, Asteroid):
                    if isinstance(asteroid, Boss):
                        self.shoot(asteroid.position.x - asteroid.width, asteroid.position.y - 1.3 * asteroid.height, enemy = True)
                        self.shoot(asteroid.position.x + asteroid.width, asteroid.position.y - 1.3 * asteroid.height, enemy = True)
                    else:    
